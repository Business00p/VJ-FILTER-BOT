...
async def filter_episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("♻️𝐒𝐞𝐧𝐝 𝐀𝐥𝐥", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("🎭 ʟᴀɴɢᴜᴀɢᴇs ✨", callback_data=f"languages#{key}"),
            InlineKeyboardButton("🚩 ʏᴇᴀʀs ⌛", callback_data=f"years#{key}")
        ])
    else:
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("♻️ 𝐒𝐞𝐧𝐝 𝐀𝐥𝐥", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("🎭 ʟᴀɴɢᴜᴀɢᴇs ✨", callback_data=f"languages#{key}"),
            InlineKeyboardButton("🚩 ʏᴇᴀʀsn⌛", callback_data=f"years#{key}")
        ])
    if lang != "homepage":
        req = query.from_user.id
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/tvshowpublic")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @kingvj01
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Youtube Channel', url="https://youtube.com/@techbro-kd8yk?si=sytxunP_S39I8HyI")
        ],[
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="https://t.me/TvShOw076")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
